...
> **"계획 중심 커리어 패스 에디터, 커리어 실행으로 이어지는 학생 주도 플랫폼"**  
...
┌──────────────────────────────────────────────────────────────────┐
│                                                                  │
│   CareerPath = 계획 중심 커리어 패스 에디터 + 커리어 실행 연결             │
│                                                                  │
│   핵심 철학:                                                      │
...
| **핵심 차별점** | 계획 중심 커리어 패스 에디터 + 단계 분해 + 커리어 실행 연계 (시장 유일) |
...
| 기존 불안 | AI 시대의 불안 | CareerPath 해결 |
|----------|-------------|--------------|
| "결과물을 어떻게 만들지?" | "AI가 다 만드는데 내가 뭘 해야 하지?" | 기획하는 과정이 진짜 실력 |
| "포트폴리오가 없다" | "AI가 만든 것을 내 것이라 할 수 있나?" | 설계 과정이 나만의 포트폴리오 |
| "방향을 모르겠다" | "AI한테 물어봐도 내 상황에 맞지 않다" | 커리어 패스 좁히기 + 커리어 실행 그룹에서 함께 실행 |

---

## 3. 솔루션 (Solution)

### 3-1. CareerPath의 핵심 가치
...
CareerPath:
  클릭 → 패스 선택 ──── 나만의 것으로 좁히기 ──── 커리어 실행 모임에서 함께 실행
       ↑ 누구나 즉시 시작 가능            ↑ 같은 꿈 동료와 함께
```

### 3-2. 메인 기능: 커리어 패스 설계 + 커리어 실행 모임
...
graph TD
    ...
    subgraph "STEP 3: 커리어 실행에서 함께 실행"
        H --> I["커리어 실행 피드/탐색 탭 진입"]
...
- **템플릿 운영**: 가이드는 `커리어 패스` + `커리어 실행(피드/탐색)`에서 이미 제공한 템플릿 중심으로 운영
- **자료실 분리 운영**: 자료실은 `커리어 경험` 영역에서 고입·대입·취업 정보를 통합 가이드하는 별도 축으로 운영
- **명칭/구조 분리**: 사용자 명칭은 `커리어 실행`, 코드 경로는 현재 `dreammate` 구조를 유지하여 안정적으로 확장

### 3-2-2. Notion/Jira 대비 차별화

| 항목 | 일반 에디터 (Notion/Jira 등) | CareerPath |
|------|-------------------------------|-----------|
| 제품 성격 | 범용 문서/업무 관리 | **진로 한정 계획·실행 엔진** |
| 시작 방식 | 빈 문서에서 직접 설계 | **커리어 패스 템플릿 즉시 시작** |
| 실행 연결 | 수동 태스크 연결 | **단계 분해 후 커리어 실행 자동 연결** |
...
### 3-4. CareerPath vs 기존 방식 비교

| 항목 | 입시 컨설턴트 | 유학원 | 커리어넷 | **CareerPath** |
|------|-------------|--------|---------|---------------|
| **커리어 패스 생성** | 수동, 300만원 | 없음 | 없음 | ✅ **클릭 한 번** |
| **나만의 계획 커스텀** | 1회성 | 없음 | 없음 | ✅ **무제한 좁히기** |
| **함께 실행 (모임)** | 없음 | 없음 | 없음 | ✅ **커리어 실행 그룹** |
...
> 그러나 CareerPath는 학생들이 생산하는 계획·실행·회고 데이터를 템플릿과 결합해  
...
### 4-3. CareerPath 핵심 차별점

```
┌─────────────────────────────────────────────────────────────┐
│              CareerPath만의 3가지 해자 (Moat)                  │
├─────────────────────────────────────────────────────────────┤
│                                                             │
...
| 차별점 | 커리어넷 | 메이저맵 | 드림어필 | 에듀캔버스 | 베어러블 | **CareerPath** |
|--------|---------|---------|---------|----------|---------|-------------|
| 적성검사 | ✅ | ✅ | ❌ | ✅ | ❌ | ✅ |
| 직업 시뮬레이션 | ❌ | ❌ | ❌ | ✅ (3D) | ❌ | ✅ (별 탐험) |
| **클릭으로 패스 생성** | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ **(유일)** |
...
> CareerPath의 기본은 **학생이 피드·검색에서 시작**하고, **가이드 템플릿으로 계획**한 뒤, **그룹에서 실행을 이어가는 구조**입니다.
...
#### 학교·기관 연계 Pain Point → CareerPath 해결 매핑

| # | 연계 운영 어려움 | 현재 해결책 | 문제점 | CareerPath 해결 |
|---|-------------|-----------|--------|---------------|
...
| ❹ | 학생 참여도 저하 | 적성검사 1회 | 검사 후 흥미 상실 | 커리어 패스 좁히기 + 커리어 실행 모임 + 커뮤니티 → 지속 참여 |
| ❺ | 외부 체험 한계 (연 1~2회) | 직업 체험처 직접 섭외 | 비용·시간 한계 | 커리어 실행 Zoom 세미나 (현직자 온라인 강연) + 프로젝트 그룹 |
...
│                                                            │
│  배경: 서울 거주, 상위권 학생                                 │
│  상황: 친구가 공유한 피드에서 CareerPath를 발견                   │
...
│                                                            │
│  확장:                                                         │
│  → 커리어 실행 커뮤니티 → 의료계 진로 탐색반 그룹 가입 (멘토링)   │
...
│  "커리어 실행 모임과 커뮤니티 그룹에서 같은 꿈 동료와 함께 성장"   │
...
| **커리어 실행 연결** | 피드·검색·그룹 협업으로 연결 | 기본 무료 |
| **자료 연동** | 커리어 경험 자료실 템플릿 연결 | 기본 무료 |
...
| 시기 | 마일스톤 | 핵심 지표 |
|------|---------|---------|
| **2026 Q1** | MVP 안정화 + 커리어 실행 완성 (피드/검색/그룹 핵심 기능) | 앱 완성도, 계획 생성률 |
| **2026 Q2** | 200개 패스 + 백엔드 연동 | 가입 5,000명, 활성 모임 10개 |
| **2026 Q3** | 광고 시스템 + 세미나 캠페인 + AI 멘토 고도화 | 가입 30,000명, 월 광고 매출 |
| **2026 Q4** | 커리어 실행 고도화 + 기관 연계 확장 | 활성 그룹 100개, 기관 연계 수익 |
...
| 라운드 | 시기 | 금액 | 용도 |
|--------|------|------|------|
| **Pre-Seed** | 2026 Q1 | **2억원** | 200개 패스 완성 + 커리어 실행 고도화 + 팀 빌딩 |
...
│                                                               │
│                        CareerPath                              │
│    "계획 중심 커리어 패스 에디터 + 커리어 실행으로 함께 실행"         │
│                                                               │
├───────────────────────────────────────────────────────────────┤
│                                                               │
│  핵심 철학:                                                    │
...
│  고객: 중고등학생 260만명 + 학부모                              │
│  경쟁 우위: 계획 중심 에디터 + 단계 분해 + 커리어 실행 연계 (시장 유일)│
│                                                               │
│  투자 요청: Pre-Seed 2억원                                     │
│                                                               │
└───────────────────────────────────────────────────────────────┘
