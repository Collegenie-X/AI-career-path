...
- `useSearchParams` 기반 딥링크 복원: `starId`, `jobId`로 탐색 상태와 모달을 즉시 재구성
- 황금각 기반 별 배치: `(i * 137.5) % 100` 분포로 배경 별이 자연스럽게 퍼지도록 생성
- 고입 추천 분기 흐름: 예/아니오 트리로 카테고리 id를 추론해 맞춤 고교 유형 추천
- 스와이프 카드 전환: 방향(`left/right`) 상태와 타이머를 결합해 애니메이션 후 데이터 반영

### 구성된 시스템

- 프레젠테이션 계층: `app/jobs/**` UI 컴포넌트(탭, 카드, 모달, 인터랙션)
- 설정/라벨 계층: `config.ts`에서 라벨·색상·상수 중앙관리
- 데이터 계층: `data/*.json` 정적 콘텐츠(직업, 별, 고교, 커리어 루트)
- 상태 계층: 페이지 단위 `useState` + 뷰 상태 머신(`AdmissionViewState`) 기반 화면 전환
- 공통 UI 기반: `CareerPathStyleDialog`, `TabBar` 등 재사용 컴포넌트로 일관된 UX 유지
