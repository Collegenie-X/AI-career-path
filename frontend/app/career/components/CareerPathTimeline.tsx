...
import {
  Map, Pencil, Trash2, Share2, Download, Calendar,
  Target, ChevronDown, ChevronUp, Plus, Trophy,
  Activity, Award, Sparkles, CheckCircle2, Circle
} from 'lucide-react';
import { LABELS, ITEM_TYPES, GRADE_YEARS } from '../config';
import type { CareerPlan, YearPlan, PlanItem } from './CareerPathBuilder';
...
function YearCard({
  ...
  const typeBreakdown = yearPlan.items.reduce(
    (acc, item) => { acc[item.type] = (acc[item.type] || 0) + 1; return acc; },
    {} as Record<string, number>
  );

  const byMonth = yearPlan.items
    .slice()
    .sort((a, b) => (a.months?.[0] ?? 0) - (b.months?.[0] ?? 0));
      ...
      >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {Object.entries(typeBreakdown).map(([type, count]) => {
                const conf = ITEM_TYPES.find(t => t.value === type)!;
                return (
                  <span key={type} className="text-[10px]" style={{ color: conf.color }}>
                    {conf.emoji} {count}
                  </span>
                );
              })}
              {yearPlan.items.length === 0 && (
                <span className="text-[10px] text-gray-500">항목 없음</span>
              )}
            </div>
          </div>
        ...
        {expanded && (
          <div className="px-4 pb-4 space-y-3">
              ...
              <div className="space-y-2">
                ...
                {byMonth.map(item => (
                  <ItemCard
                    ...
                    item={item}
                    color={planColor}
                    onRemove={() => onRemoveItem(yearPlan.gradeId, item.id)}
                  />
                ))}
              </div>
            )}
            ...
            {/* Empty state */}
            {yearPlan.items.length === 0 && yearPlan.goals.length === 0 && (
...
function WBSChart({ plan }: { plan: CareerPlan }) {
  const allItems = plan.years.flatMap(y =>
    y.items.map(item => ({ ...item, gradeLabel: y.gradeLabel, gradeId: y.gradeId }))
  );
  ...
  const handleRemoveItem = (gradeId: string, itemId: string) => {
    ...
    setLocalPlan({
      ...
      years: localPlan.years.map(y =>
        y.gradeId === gradeId
          ? { ...y, items: y.items.filter(it => it.id !== itemId) }
          : y
      ).filter(y => y.items.length > 0 || y.goals.length > 0),
    });
  };
