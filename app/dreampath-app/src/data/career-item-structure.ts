...
function inferCategoryTags(item: BaseCareerItemLike): CareerItemCategoryTag[] {
  ...
  if (item.type === 'award' || /수상|대회|올림피아드|입상|경진/i.test(text)) tags.add('award');
  if (/논문|연구|r&e|탐구|리서치|보고서/i.test(text)) tags.add('paper');
  if (/인턴|intern|현장실습/i.test(text)) tags.add('intern');
  if (/봉사|volunteer|사회공헌/i.test(text)) tags.add('volunteer');
  if (/캠프|camp|부트캠프|summer program/i.test(text)) tags.add('camp');
