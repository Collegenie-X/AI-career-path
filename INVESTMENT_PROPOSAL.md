...
│   ② 나만의 범위·주제로 점점 좁혀가기 (초4~고2 학년별 로드맵)          │
│                                                                  │
│   활성화 기능 (런치패드 — 모임 만들기 & 참여):                      │
│   · 진로 탐색 / 커리어 설계 / 실행·도전 / 자격·수상 4가지 모임 유형   │
│   · 온라인(Zoom) · 오프라인 · 온·오프 하이브리드 진행 방식           │
│   · 학교 동아리 + 스터디·프로젝트팀·멘토링·동아리 4가지 그룹 유형     │
│   · 커뮤니티 접근 제한 (초대 코드 또는 가입 요청 승인)               │
│                                                                  │
│   수익 모델 3가지:                                                │
...
│                                                            │
│  📅 1차시: 적성검사 + 8개 별(왕국) 탐험                       │
│  → 학생들이 각자 앱에서 RIASEC 검사 후 관심 별 탐색            │
│                                                            │
│  📅 2~4차시: 커리어 패스 선택 + 나만의 계획 좁히기              │
│  → 별 클릭 → 직업 선택 → 학년·항목 유형으로 커스텀             │
│  → 교사는 대시보드에서 학급 전체 진행 현황 확인                  │
│                                                            │
│  📅 5~8차시: 런치패드 학급 모임                                │
│  → 진로 탐색 / 커리어 설계 / 실행·도전 / 자격·수상 모임 참여    │
│  → Zoom 온라인 또는 오프라인 진행 선택                         │
│                                                            │
│  📅 9~16차시: 프로젝트 발표 + 커뮤니티 확장                    │
│  → 학교 동아리 또는 스터디·프로젝트팀·멘토링 그룹 가입           │
│  → 타 학교 학생과 런치패드 모임 연결                           │
│                                                            │
│  교사 전환 포인트:                                            │
...
| ❹ | 학생 참여도 저하 | 적성검사 1회 | 검사 후 흥미 상실 | 커리어 패스 좁히기 + 런치패드 모임 + 커뮤니티 → 지속 참여 |
| ❺ | 외부 체험 한계 (연 1~2회) | 직업 체험처 직접 섭외 | 비용·시간 한계 | 런치패드 Zoom 세미나 (현직자 온라인 강연) + 프로젝트 그룹 |
...
│                                                            │
│  학교 수업 여정:                                             │
│  📅 창체 시간: 탐구의 별 🔬 → 의사 커리어 패스 클릭            │
│  → 고1~고2 학년 범위 + 과학 탐구 활동 특화로 좁히기            │
│  → 교사가 대시보드에서 활동 기록 확인 → 생기부 반영             │
│                                                            │
│  수업 밖 확장:                                               │
│  → 런치패드 커뮤니티 → 의료계 진로 탐색반 그룹 가입 (멘토링)    │
...
│  "런치패드 모임과 커뮤니티 그룹에서 같은 꿈 동료와 함께 성장"    │
...
| **나만의 커스텀** | 활동·수상·작품·자격증 항목 추가 | ⛔ 잠금 | ✅ 무제한 커스텀 |
| **런치패드 모임** | 관련 모임 참여 | 무료 모임 1개 | ✅ 무제한 모임 참여 + 생성 |
...
> **런치패드 모임 + 커뮤니티 그룹으로 학생들의 실행력을 활성화**하면서 3가지 수익축을 **점진적으로 확장**합니다.

---

### 7-2. 구독 플랜

| 플랜 | 월 가격 | 커리어 패스 | AI 멘토 | 런치패드 모임 | 커뮤니티 | 대상 |
|------|--------|-----------|--------|-------------|---------|------|
| **Free** | 0원 | 200개 탐색 + 기본 사용 | 5회/일 | 무료 모임 참여 1개 | 탐색만 | 탐색 단계 학생 |
| **Explorer** | **9,900원** | 나만의 패스 무제한 저장 | 50회/월 | 무제한 참여 + 모임 생성 3개 | 그룹 가입 | 진로 설계 시작 학생 |
| **Pioneer** | **19,900원** | 커스텀 패스 제작 + 공유 | 무제한 | 무제한 참여·생성 + 프리미엄 모임 | 그룹 생성 + 학교 동아리 | 적극적 입시 준비 학생 |

---

### 7-3. 런치패드 — 모임 유형 상세 설계

> **커리어 패스를 설계한 뒤, 혼자 실행하면 포기한다. 런치패드는 같은 꿈을 가진 동료끼리 모여서 함께 실행하는 활성화 엔진이다.**

#### 모임 유형 4가지 (SessionType)

| 유형 | 설명 | 기본 방식 |
|------|------|---------|
| **진로 탐색** 🔍 | 직업·진로 알아보기, 현직자 토크 | 온라인 |
| **커리어 설계** 🗺️ | 커리어 패스·로드맵 작성 실습 | 온라인 |
| **실행·도전** ⚡ | 공모전·프로젝트 기획 및 실행 | 온라인 |
| **자격·수상 준비** 🏆 | 자격증 스터디, 포트폴리오 피드백 | 온라인 |

#### 진행 방식 3가지 (ModeType)

| 방식 | 설명 |
|------|------|
| **온라인 (Zoom)** | Zoom 링크로 진행 |
| **오프라인** | 학교·장소에서 오프라인 진행 |
| **온·오프 하이브리드** | Zoom + 오프라인 병행 |

#### 목적 태그 8가지 (PurposeTagKey)

커리어 기획 🗺️ · 프로젝트 🚀 · 자격증 📜 · 수상·공모전 🏆 · 포트폴리오 📁 · 네트워킹 🤝 · 직업인 토크 🎤 · 스터디 📖

#### 빠른 생성 템플릿 6가지

| 템플릿 | 유형 | 방식 |
|--------|------|------|
| IT/개발 진로 탐색 💻 | 진로 탐색 | 온라인 |
| 디자인 커리어 설계 🎨 | 커리어 설계 | 온라인 |
| 이공계 진로 탐색 🔬 | 진로 탐색 | 온라인 |
| 공모전 도전 🏆 | 실행·도전 | 온라인 |
| 자격증 스터디 📜 | 자격·수상 준비 | 온라인 |
| 학교 동아리 활동 🏫 | 커리어 설계 | 오프라인 |

---

### 7-4. 커뮤니티 — 그룹 유형 상세 설계

> **런치패드 모임이 1회성 실행이라면, 커뮤니티 그룹은 지속적으로 함께 성장하는 공간이다.**
...
├── 학교 동아리 (SchoolClub)
│   └── 진로 교사가 개설, 학교 단위 운영
│       예) 서울고 진로탐색 동아리, 강남중 IT 동아리
│
└── 그룹 (LaunchpadGroup)
    ├── 스터디 📖 — 함께 학습하고 지식을 나누는 그룹
    ├── 프로젝트팀 ⚡ — 공모전·프로젝트를 함께 실행하는 팀
    ├── 멘토링 🎓 — 선배·현직자에게 조언을 받는 그룹
    └── 동아리 🏫 — 학교 내 진로·커리어 동아리
```
...
접근 게이트 표시
    ├── 방법 A: 초대 코드 입력 → 즉시 접근
    └── 방법 B: 가입 요청 (자기소개 작성) → 관리자 승인
    ↓ 접근 허용
학교 동아리 / 그룹 이용 가능
    ↓ 동아리·그룹 카드 클릭 (미가입)
JoinRequestDialog 표시
    ↓ 가입 확인
동아리·그룹 상세 진입 + 소속 모임 목록 확인
```
...
| 카테고리 | 이모지 | 색상 | 설명 |
|---------|--------|------|------|
| **스터디** | 📖 | #3B82F6 | 함께 학습하고 지식을 나누는 그룹 |
| **프로젝트팀** | ⚡ | #FBBF24 | 공모전·프로젝트를 함께 실행하는 팀 |
| **멘토링** | 🎓 | #6C5CE7 | 선배·현직자에게 조언을 받는 그룹 |
| **동아리** | 🏫 | #22C55E | 학교 내 진로·커리어 동아리 |
...
graph LR
    ...
    subgraph "Phase 2: 학교 안 Aha Moment"
        C --> D["8개 별 탐험 →<br/>커리어 패스 즉시 생성"]
        D --> E["학급 런치패드<br/>모임 → 팀 프로젝트"]
        E --> F["커뮤니티 그룹<br/>가입 → 지속 활동"]
    end

    subgraph "Phase 3: 학교 밖 확장 (Convert)"
        F --> G["타 학교 런치패드<br/>모임 참여 (확장)"]
        G --> H["AI 멘토링 구독<br/>(수익축 2)"]
        G --> H2["외부 커리어 패스 구매<br/>(수익축 1)"]
    end
```
...
| 시기 | 마일스톤 | 핵심 지표 |
|------|---------|---------|
| **2026 Q1** | MVP 안정화 + 런치패드 완성 (모임 4유형 + 커뮤니티 그룹 4유형) | 앱 완성도, 런치패드 기능 완비 |
| **2026 Q2** | 200개 패스 + 백엔드 연동 | 가입 5,000명, 활성 모임 10개 |
| **2026 Q3** | 결제 + AI 멘토 + 유료 모임 | 가입 30,000명, MRR 3,000만원 |
...
| 라운드 | 시기 | 금액 | 용도 |
|--------|------|------|------|
| **Pre-Seed** | 2026 Q1 | **2억원** | 200개 패스 완성 + 런치패드 고도화 + 팀 빌딩 |
...
│                                                               │
│  솔루션 (활성화):                                              │
│  · 런치패드 — 4가지 유형 × 3가지 방식 모임 만들기 & 참여          │
│    → 진로 탐색·커리어 설계·실행·도전·자격·수상 모임              │
│    → 온라인(Zoom)·오프라인·하이브리드 진행                       │
│  · 커뮤니티 — 학교 동아리 + 스터디·프로젝트팀·멘토링·동아리 그룹  │
│    → 초대 코드 또는 가입 요청 승인 기반 신뢰 네트워크             │
│                                                               │
│  서브 기능:                                                    │
│  · AI 프로젝트 멘토링 (실행 지원)                               │
│                                                               │
...
│  고객: 중고등학생 260만명 + 학부모                              │
│  경쟁 우위: 커리어 패스 즉시 생성 + 런치패드 + 커뮤니티 (시장 유일)│
│                                                               │
│  투자 요청: Pre-Seed 2억원                                     │
│                                                               │
└───────────────────────────────────────────────────────────────┘
